...
def extract_opening_player_flight(mach, staging, actors, rotation, response):
    ...
    try:
        ...
        if rotation['provenance'][0]['offset']!=origin+LAYOUTS[arch]['rotation_anchor'][0]:return {}
        if response['provenance'][0]['offset']!=origin+LAYOUTS[arch]['response_anchor'][0]:return {}
        at=mach.text['address']+origin-mach.slice_offset-mach.text['offset']
        proof={}
        for key,(delta,size,raw) in LAYOUTS[arch].items():
            found=section_bytes(mach,at+delta,size,b'__text')
