...
func build(owner: RefCounted,library: RefCounted,visuals: RefCounted,bindings: RefCounted) -> bool:
	clear()
	if not (owner is State or owner is FullHold) or owner.presentation_identity()==null:return reject("Damage geometry requires its configured flight owner")
	...
	for kind in ["trail","smoke","fire","burst"]:
		...
		for key in state.owners:
			...
			if descriptor.is_empty() or not Materials.supports(descriptor) or descriptor.render_type!=(1 if kind=="smoke" else 2) or descriptor.parameter_bits.map(func(value):return int(value))!=[0,3240099840,0,0] or descriptor.texture_paths[0].is_empty():return reject("Unsupported damage sprite material")
...
func prepare_world(owner: RefCounted,world: Dictionary,camera_pose: Variant) -> Dictionary:
	error=""
	if not (owner is State or owner is FullHold) or _owner_identity==null or owner.presentation_identity()!=_owner_identity:return failed("Damage sprites follow one configured flight")
	var state: Dictionary=owner.snapshot()
	...
	var elapsed: Variant=world.get("encounter",{}).get("elapsed_ms") if owner is FullHold else world.get("elapsed_ms")
	if world.get("damage_particles")!=state or elapsed!=state.elapsed_ms:return failed("Damage sprite presentation requires its current world clock")
