extends RefCounted
## Native continuous translation in original content coordinates, local +Z.
...
func advance(pose: Transform3D, throttle: float, seconds: float) -> Transform3D:
	error = ""
	...
	if not is_finite(throttle) or throttle < 0.0 or throttle > 1.0 \
		...
		error = "Invalid ordinary cruise throttle or elapsed simulation time"
		return pose
	...
	var next := pose.origin + forward * (_speed_per_second * throttle * seconds)
	if not forward.is_finite() or forward.is_zero_approx() or not next.is_finite():
