...
func evaluate(actor: Dictionary, amount: Variant, nonplayer: Variant, random_state: Dictionary, display_available: bool) -> Dictionary:
	error=""
	if _state.is_empty() or not Vitals.integer(amount) or not nonplayer is bool:return fail("Provocation requires configured traffic and valid damage attribution")
	for key in ["base_content_id","binding_id","campaign_cursor"]:
		if actor.get(key)!=_state[key]:return fail("Provocation belongs to another encounter")
	var id: Variant=actor.get("actor_id")
	if not id is int or id<0 or id>=_state.requested_damage.size():return fail("Provocation actor is outside this faction population")
	var expected_kind:=int(_state.actor_kinds[id]) if _state.has("actor_kinds") else int(_rules.actor_kind)
	if actor.get("actor_kind")!=expected_kind:return fail("Provocation actor affiliation changed")
	for key in ["active","damage_allowed","hostile","forced_hostile"]:
		if not actor.get(key) is bool:return fail("Provocation requires current actor permissions and hostility")
	if actor.forced_hostile!=_state.forced_hostile[id] or not Vitals.integer(actor.get("max_hull")) or not Vitals.integer(actor.get("vitals",{}).get("hull")):return fail("Provocation actor disagrees with its retained state")
	...
	if faction_eligible and actor.active and actor.damage_allowed and actor.vitals.hull>0 and not nonplayer and (not actor.hostile or actor.forced_hostile):
		...
		if exceeds(total,actor.max_hull,float(_rules.faction_fraction)):
			for other in next._state.forced_hostile.size():
				if not next._state.has("actor_kinds") or next._state.actor_kinds[other]==expected_kind:next._state.forced_hostile[other]=true
