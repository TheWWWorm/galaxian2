...
const PATHS=["resources/data/assets/main/3d/meshes/fx/projectile_002_anim_add.aem","resources/data/assets/main/3d/meshes/fx/projectile_019_anim_add.aem"]
var error:=""
...
func configure(bindings: RefCounted, library: RefCounted, world: Dictionary) -> bool:
	...
	for entry in inputs:
		...
		# JSON numbers and native fixture integers have distinct Array membership.
		if index<0: index=rules.item_ids.find(int(weapon.item_id))
		if index<0 or weapon.get("kind")!=rules.kinds[index] or weapon.get("category")!=0: return reject("Projectile model is outside the fresh ordinary weapon set")
		for key in ["base_content_id","binding_id"]:
		...
		if path!=PATHS[index]:return reject("Unsupported projectile model mapping")
		if not metadata.has(id):
			...
			if timing.is_empty() or timing.end_ms<=0:return reject("Unsupported projectile animation range")
			metadata[id]=timing
		var timing: Dictionary=metadata[id]
		models.append({"key":entry.key,"item_id":int(weapon.item_id),"kind":int(weapon.kind),"capacity":int(weapon.projectile_capacity),"model_id":id,"resource":path,"start_ms":timing.start_ms,"end_ms":timing.end_ms,"time_ms":timing.start_ms,"playing":true})
