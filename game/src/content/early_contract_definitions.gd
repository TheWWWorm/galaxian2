...
static func parameters(data: Variant) -> bool:
	...
	if data.has("acceptance") and not Equal.equal_value(data.acceptance,ACCEPTANCE):return false
	...
	if data.has("base_station_stock") and (not data.has("station_generation") or not BaseStock.parameters(data.base_station_stock)):return false
...
static func validate(data: Variant,source_bytes: int,arch: String,arrival: Dictionary,travel: Dictionary) -> String:
	...
	if data.has("acceptance"):spans.merge(ACCEPTANCE_SPANS)
	if data.has("generation"):spans.merge(Contacts.SPANS)
	if data.has("delivery_results"):spans.merge(Delivery.SPANS)
	if data.has("encounter_construction"):
		...
		spans.merge(Encounters.SPANS)
	if data.has("ship_combat"):spans.merge(ShipCombat.SPANS)
	if data.has("ship_lifecycle"):spans.merge(ShipLifecycle.SPANS)
	if data.has("flight_results"):spans.merge(FlightResults.SPANS)
	if data.has("junk_lifecycle"):spans.merge(Junk.SPANS)
	if data.has("world_initialization"):spans.merge(World.SPANS)
	if data.has("station_lounges"):spans.merge(Lounges.SPANS)
	if data.has("station_generation"):spans.merge(Stock.SPANS)
	if data.has("base_station_stock"):spans.merge(BaseStock.SPANS)
	if data.has("base_navigation"):
		if not travel.has("alioth_arrival"):return "Base contracts lack their supported Alioth arrival"
		spans.merge(Navigation.SPANS)
	if data.has("lounge_presentation"):spans.merge(LoungePresentation.SPANS)
