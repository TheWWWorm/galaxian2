...
static func validate(data: Variant, executable_bytes: int, architecture: String, npc: Dictionary) -> String:
	...
	for key in SPANS[architecture]:
		...
		if not Fonts.extent(span,"offset","bytes",[rule[1]],executable_bytes) or int(span.offset)!=update+int(rule[0]): return "Disconnected NPC death accounting declaration"
