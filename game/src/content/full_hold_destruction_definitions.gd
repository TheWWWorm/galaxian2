...
static func validate(data: Variant, source_bytes: int, arch: String, arrival: Dictionary, control: Dictionary, actors: Dictionary) -> String:
	...
	if data.provenance.size()!=SPANS.size():return "Invalid second-trip destruction provenance"
	for key in SPANS:
		var span: Variant=data.provenance.get(key);var rule: Array=SPANS[key]
		if not Fonts.extent(span,"offset","bytes",[rule[1]],source_bytes) or int(span.offset)!=int(origin.offset)+int(rule[0]):return "Invalid second-trip destruction extent: "+key
	return ""
