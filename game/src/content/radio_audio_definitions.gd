...
static func parameters(data: Variant, count: int = 23) -> bool:
	if count not in [3, 23]: return false
...
static func validate(data: Variant, executable_bytes: int, architecture: String, dialogue: Dictionary, fonts: Dictionary, audio: Dictionary) -> String:
	...
	for key in spans:
		...
		if not Fonts.extent(row,"offset","bytes",[spans[key][1]],executable_bytes) or int(row.offset)!=int(origin)+int(spans[key][0]):return "Disconnected radio voice declaration: "+key
