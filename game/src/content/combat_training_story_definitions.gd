...
static func parameters(data: Variant) -> bool:
	if not data is Dictionary or data.size()!=VALUES.size()+1+int(data.has("station_return"))+int(data.has("navigation")) or not data.get("provenance") is Dictionary:return false
...
static func station_return_parameters(rules: Dictionary) -> bool:
	for key in RETURN_VALUES:
		if not Equal.equal_value(rules.get(key),RETURN_VALUES[key]):return false
