...
static func validate(data: Variant, bytes: int, arch: String, arrival: Dictionary, actors: Dictionary, environment: Dictionary) -> String:
	...
	for key in SPANS[arch]:
		...
		if not Fonts.extent(span,"offset","bytes",[rule[1]],bytes) or int(span.offset)!=int(origin.offset)+int(rule[0]):return "Disconnected rescue actor construction declaration"
	return ""
