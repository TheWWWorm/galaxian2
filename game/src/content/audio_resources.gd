...
func prepare(id: int) -> Dictionary:
	...
	if int(p.mode) not in [0x180008,0x280010] or p.pitch!=0 or p.pitch_random!=0 or p.volume_random!=0 or not Definitions.integer(p.max_playbacks,1,3) or p.max_playbacks_behavior!=1 or int(p.flags) not in [0,0x80000]:return unavailable(id,"This event needs additional native playback behavior")
...
func prepare_static(id: int, event: Dictionary) -> Dictionary:
	...
	if int(sound.flags) not in [0,1] or sound.flags2!=0 or sound.loop_count!=-1 or sound.auto_pitch!=0 or sound.fine_tune!=0 or sound.volume<0 or sound.volume>4 or float(sound.fade_in) not in [-1.0,0.0] or float(sound.fade_out) not in [-1.0,0.0] or sound.x!=0 or sound.width!=1:return unavailable(id,"This sound requires native scheduling or parameter automation")
	...
	var definition:=cached_playlist(int(sound.sound_def),sound.flags==0)
	if definition.is_empty():return {}
	if definition.has("unsupported"):return unavailable(id,definition.unsupported)
	var result:=event_header(event)
	result.gain*=float(sound.volume);result.looping=sound.flags==0
...
func load_sample(entry: Dictionary, looping: bool) -> Dictionary:
	...
	var stream: AudioStream=bank.stream(int(entry.index),looping)
...
func event_header(event: Dictionary) -> Dictionary:
	var p: Dictionary=event.properties
	return {"id":int(event.id),"name":event.path,"gain":float(p.volume),
