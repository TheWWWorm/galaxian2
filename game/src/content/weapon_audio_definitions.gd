...
static func validate(data: Variant, executable_bytes: int, architecture: String, weapons: Dictionary, actors: Dictionary, staging: Dictionary) -> String:
	...
	for key in spans:
		...
		if not Fonts.extent(row,"offset","bytes",[spans[key][1]],executable_bytes) or int(row.offset)!=int(origin)+int(spans[key][0]):return "Disconnected weapon audio declaration: "+key
