...
static func validate_audio(data: Variant, executable_bytes: int, architecture: String, npc: Dictionary) -> String:
	...
	if data.provenance.size()!=spans.size():return "Invalid NPC destruction audio provenance"
	for key in spans:
		var span: Variant=data.provenance.get(key)
		if not Fonts.extent(span,"offset","bytes",[spans[key][1]],executable_bytes) or int(span.offset)!=update+int(spans[key][0]):return "Disconnected NPC destruction audio declaration"
	return ""
...
static func validate(data: Variant, executable_bytes: int, architecture: String, npc: Dictionary) -> String:
	...
	for key in SPANS[architecture]:
		...
		var span: Variant=data.provenance.get(key)
		if not Fonts.extent(span,"offset","bytes",[rule[1]],executable_bytes) or int(span.offset)!=update+int(rule[0]): return "Disconnected NPC destruction declaration"
	return ""
