...
static func shared_weapon(rules: Dictionary,cursor: int,rank: int,difficulty: float,faction: int,rival:=false) -> Dictionary:
	if not Equal.equal_value(rules,VALUES.weapons) or cursor not in ([13,14]+FlightStages.FREE) or rank<0 or rank>20 or difficulty not in [0.5,1.0] or faction not in [0,1,2,3,8]:return {}
...
static func scaled_parameters(rules: Dictionary,cursor: int,rank: int,difficulty: float,rival:=false) -> Dictionary:
	...
	if not Equal.equal_value(rules,VALUES.weapons) or cursor not in FlightStages.FACTIONS or rank<0 or rank>20 or difficulty not in [0.5,1.0]:return {}
