...
func check_group(bindings: RefCounted) -> void:
	...
	check(not group.update(751 if not bindings.fast_forward.is_empty() else 151,Vector3.ZERO,1,false) and group.snapshot()==before,"Invalid frame changed scenery clock")
	...
	var sibling: RefCounted=forked.fork_for_frame()
	check(forked.update(100,Vector3.ZERO,1,false) and forked.snapshot().counter_ms==250,"Scenery fork lost the retained periodic accumulator")
	check(forked.refresh(Vector3(70000,0,0),1) and forked.snapshot().selections[0].level==0,"Scenery fork did not use its configured field")
	check(sibling.snapshot()==before,"Scenery fork refresh changed its sibling")
	var accepted: Dictionary=forked.snapshot()
	check(not forked.refresh(Vector3.ZERO,NAN) and forked.snapshot()==accepted,"Rejected scenery detail changed a fork")
	check(sibling.snapshot()==before and sibling.error.is_empty(),"Rejected scenery fork changed its sibling or error")
	forked.clear()
	check(sibling.snapshot()==before and sibling.refresh(Vector3.ZERO,1),"Clearing a scenery fork erased another owner's positions")
