...
func check_synthetic() -> void:
	...
	check(initial.get("radar_marked_actor")==false,"Base-constructed NPC was given an unproved radar music marker")
	...
	check(impact.radar_marked_actor==false,"Ordinary damage or death invented the world-population music marker")
	...
	check(actor.snapshot().radar_marked_actor==false,"Reconfiguration retained a radar marker")
...
func check_profile(content: String, binding_path: String) -> void:
	...
	for difficulty in [0.0,0.5,1.0,1.5]:
		...
		for id in 3:
			...
			check(state.radar_marked_actor==false,"Opening NPC source constructor invented a marked battle cue")
