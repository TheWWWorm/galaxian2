"""Relocation and rejection checks for imported convoy choreography."""
import copy
...
from gof2_content import convoy_capture as reader

class ConvoyCaptureTests(unittest.TestCase):
    def test_relocated_and_changed_source(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_convoy_capture(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_convoy_capture(changed,arrival),({},{}),name)
                values['arrival']['reward']=999
                self.assertEqual(reader.extract_convoy_capture(mach,arrival)[0],reader.VALUES)

    def test_other_architecture_stays_absent(self):
        mach,arrival,_=declaration_fixture(reader.LAYOUTS)
        mach.architecture='armv7'
        self.assertEqual(reader.extract_convoy_capture(mach,arrival),({},{}))
