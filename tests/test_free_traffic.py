...
from gof2_content import free_traffic as reader

class FreeTrafficTests(unittest.TestCase):
    def test_proofs_and_relocation(self):
        for relocation in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_free_traffic(mach,arrival)
                self.assertEqual(values,reader.VALUES)
                self.assertEqual(set(proof),set(layouts))
                values['nivelian_boxes'][0]['offset'].clear()
                self.assertEqual(reader.extract_free_traffic(mach,arrival)[0],reader.VALUES)
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_free_traffic(changed,arrival),({},{}),name)
                mach.architecture='armv7'
                self.assertEqual(reader.extract_free_traffic(mach,arrival),({},{}))
