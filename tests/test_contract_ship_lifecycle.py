...
from gof2_content import contract_ship_lifecycle as reader


class ContractShipLifecycleTests(unittest.TestCase):
    def test_relocation_preserves_detached_declarations(self):
        for shift in (0, 0x800000):
            ...
            with patch.object(reader, 'LAYOUTS', layouts):
                values, proof = reader.extract_contract_ship_lifecycle(mach, arrival)
                ...
                values['cargo_models'].clear()
                self.assertEqual(len(reader.extract_contract_ship_lifecycle(mach, arrival)[0]['cargo_models']), 5)

    def test_each_source_mutation_disables_ship_lifecycle(self):
        ...
        with patch.object(reader, 'LAYOUTS', layouts):
            for name, (delta, _, _, _) in layouts.items():
                ...
                self.assertEqual(reader.extract_contract_ship_lifecycle(changed, arrival), ({}, {}), name)
    ...
    def test_other_architecture_stays_unsupported(self):
        ...
        self.assertEqual(reader.extract_contract_ship_lifecycle(mach, arrival), ({}, {}))
