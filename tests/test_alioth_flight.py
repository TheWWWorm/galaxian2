...
from gof2_content import alioth_flight as reader


class AliothFlightTests(unittest.TestCase):
    def test_relocation_and_source_mutations(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_alioth_flight(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_alioth_flight(changed,arrival),({},{}),name)
                values['portal']['model_id']=0
                self.assertEqual(reader.extract_alioth_flight(mach,arrival)[0],reader.VALUES)
    ...
    def test_other_architecture_remains_unavailable(self):
        ...
        self.assertEqual(reader.extract_alioth_flight(mach,arrival),({},{}))
