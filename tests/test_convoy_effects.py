"""Fail-closed and relocation checks for the original EMP declarations."""
import copy
...
from gof2_content import convoy_effects as reader

class ConvoyEffectTests(unittest.TestCase):
    def test_relocated_and_changed_source(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_convoy_effects(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_convoy_effects(changed,arrival),({},{}),name)
                values['presets'][0]['lifetime_ms']=1
                self.assertEqual(reader.extract_convoy_effects(mach,arrival)[0],reader.VALUES)

    def test_other_architecture_stays_absent(self):
        mach,arrival,_=declaration_fixture(reader.LAYOUTS)
        mach.architecture='armv7'
        self.assertEqual(reader.extract_convoy_effects(mach,arrival),({},{}))
