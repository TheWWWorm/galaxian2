...
from gof2_content import convoy_transit as reader


class ConvoyTransitTests(unittest.TestCase):
    def test_relocation_and_source_mutations(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_convoy_transit(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_convoy_transit(changed,arrival),({},{}),name)
                values['story_station_id']=0
                self.assertEqual(reader.extract_convoy_transit(mach,arrival)[0],reader.VALUES)
    ...
    def test_other_architecture_remains_unavailable(self):
        ...
        self.assertEqual(reader.extract_convoy_transit(mach,arrival),({},{}))
