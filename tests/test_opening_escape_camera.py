...
def fixture(arch, shift=0):
    ...
    staging={'escape':{'present':True},'provenance':{'initial':{'offset':bias+offset-low,'bytes':366 if arch=='x86_64' else 320}}}
    actors={}
    ...
    def test_profiles_relocation_and_data_only_output(self):
        for arch in reader.LAYOUTS:
            for shift in [0,0x700000]:
                m,s,a,*_=fixture(arch,shift);data=reader.extract_opening_escape_camera(m,s)
