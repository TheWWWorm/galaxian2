...
class BindingTests(unittest.TestCase):
    ...
    def test_pack_identity_source_matching_and_cancellation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertNotIn('ambient_combat', header)
            self.assertEqual(json.loads((pack / 'registrations.json').read_text())['freighter_destruction'], {})
            self.assertNotIn('freighter_destruction', header)
            self.assertEqual(json.loads((pack / 'registrations.json').read_text())['ambient_lifecycle'], {})
            self.assertNotIn('ambient_lifecycle', header)
