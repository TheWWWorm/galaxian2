...
import unittest
from gof2_content.text_aliases import MAC, ARM, MAC_CASES, ARM_CASES, extract_text_aliases
...
def fixture(mac, relocation=0):
    ...
    marks = {}
    cursor = start
    for token in (MAC if mac else ARM).split():
        if token.startswith('{'):
    ...
    cases = MAC_CASES if mac else ARM_CASES
