...
from gof2_content import alioth_attack as reader


class AliothAttackTests(unittest.TestCase):
    def test_relocation_and_source_mutations(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_alioth_attack(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_alioth_attack(changed,arrival),({},{}),name)
                values['population']['actors'][0]['actor_kind']=9
                self.assertEqual(reader.extract_alioth_attack(mach,arrival)[0],reader.VALUES)
    ...
    def test_other_architecture_remains_unavailable(self):
        ...
        self.assertEqual(reader.extract_alioth_attack(mach,arrival),({},{}))
