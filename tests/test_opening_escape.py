...
def fixture(arch, shift=0, rows=None):
    ...
    staging={'player_flight':{'present':True},'player_motion':{'present':True},'provenance':{'initial':{'offset':origin,'bytes':366 if arch=='x86_64' else 320}}}
    actors={}
    ...
    def test_profiles_relocation_and_data_only_output(self):
        for arch,rows in variants():
            for shift in [0,0x700000]:
                m,s,a,*_=fixture(arch,shift,rows);data=reader.extract_opening_escape(m,s)
