"""Base destinations require every static declaration and keep edition scope."""
import copy
...
from gof2_content import base_contract_navigation as reader


class BaseContractNavigationTests(unittest.TestCase):
    def test_relocation_and_source_mutations(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_base_contract_navigation(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_base_contract_navigation(changed,arrival),({},{}),name)
                values['excluded_station_ids'].clear()
                self.assertEqual(reader.extract_base_contract_navigation(mach,arrival)[0],reader.VALUES)
    ...
    def test_other_architecture_remains_unavailable(self):
        ...
        self.assertEqual(reader.extract_base_contract_navigation(mach,arrival),({},{}))
