"""Unknown station retention rules must leave the optional capability unavailable."""
import copy
...
from gof2_content import lounge_lifecycle as reader


class LoungeLifecycleTests(unittest.TestCase):
    def test_relocation_and_source_mutations(self):
        for shift in (0,0x800000):
            ...
            with patch.object(reader,'LAYOUTS',layouts):
                values,proof=reader.extract_lounge_lifecycle(mach,arrival)
                ...
                for name,(delta,_,_,_) in layouts.items():
                    ...
                    self.assertEqual(reader.extract_lounge_lifecycle(changed,arrival),({},{}),name)
                values['capacity']=0
                self.assertEqual(reader.extract_lounge_lifecycle(mach,arrival)[0],reader.VALUES)
    ...
    def test_other_architecture_remains_unavailable(self):
        ...
        self.assertEqual(reader.extract_lounge_lifecycle(mach,arrival),({},{}))
